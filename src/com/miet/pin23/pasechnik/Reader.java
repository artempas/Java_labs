...
public class Reader {
    private static ExceptionHandler exceptionHandler = new ExceptionHandler (Reader.class.getName ());
    ...
    public String GetContents ( ) {
        ...
        try {
            ...
            exceptionHandler.addException ( "Error while reading",exc,true);
    ...
    public ArrayList<Train> readTrains ( ) {
        ...
        for (String line : inp) {
            ...
            if ( line.startsWith ( "Carriage:" ) ) {
                ...
                try {
                    switch (type) {
                        case ("BOX") -> trains.get ( current_train ).getCars ( ).get ( current_carriage ).addGoods ( new BoxGoods ( line.split ( ":" )[0] ) );
                        case ("SHAKY") -> trains.get ( current_train ).getCars ( ).get ( current_carriage ).addGoods ( new ShakyGoods ( line.split ( ":" )[0] ) );
                        case ("LIQUID") -> trains.get ( current_train ).getCars ( ).get ( current_carriage ).addGoods ( new LiquidGoods ( line.split ( ":" )[0] ) );
                        case ("CAR") -> trains.get ( current_train ).getCars ( ).get ( current_carriage ).addGoods ( new Cars ( line.split ( ":" )[0] ) );
                    }
